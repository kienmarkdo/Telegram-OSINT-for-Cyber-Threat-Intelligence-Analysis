        ...
        while True:
            ...
            if len(chunk) > 0:  # Messages were returned
                if messages_collected is None:
                    ...
                    messages_collected.extend(chunk)  # Append to existing messages collected
                ...
                break
            offset_id_value = chunk[-1].to_dict()["id"]  # Set offset id as latest message collected
...
def collect_participants_new(entity: Channel | Chat | User) -> bool:
    ...
    try:
        ...
        while True:
            ...
            chunk = client(functions.channels.GetParticipantsRequest(
                channel=entity.id,
            ...
            if len(chunk) > 0:  # Partci were returned
                if participants_collected is None:
                    ...
                    participants_collected.extend(chunk)  # Append to existing messages collected
...
def collect_participants(entity: Channel | Chat | User) -> bool:
    ...
    if _get_entity_type_name(entity) == EntityName.BROADCAST_CHANNEL.value:
        print(f"Cannot collect participants in {EntityName.BROADCAST_CHANNEL.value}. Skipping participants collection...")
        return True
